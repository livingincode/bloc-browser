...
//  BlocBrowser
//
//  Created by Dean Laurea on 6/10/15.
...
- (void)loadView {
    ...
    self.textField.placeholder = NSLocalizedString(@"Search or Website URL", @"Placeholder text for search and web browser URL field");
   ...
   /* NSString *urlString = @"http://wikipedia.org";
    ...
    for (UIView *viewToAdd in @[self.webView, self.textField, self.backButton, self.forwardButton, self.reloadButton]) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    NSURL *URL = [NSURL URLWithString:URLString];
    NSRange googleSearch = [URLString rangeOfCharacterFromSet:[NSCharacterSet whitespaceCharacterSet]];
    
    if (googleSearch.location != NSNotFound) {
    ...
    if (!URL.scheme) {
    //If user didn't type http: or https:
